...
export default function CompareTray({ paints, onRemove, onClear }: CompareTrayProps) {
  ...
  return (
    <div className="border-t border-cyan/40 bg-cyan/5 px-4 py-2 flex items-center gap-3 flex-shrink-0">
      <span className="text-cyan text-[13px] tracking-widest whitespace-nowrap">
          ...
          return (
            ...
            >
              ...
              <div className="bg-surface px-1 py-0.5">
                <div className="text-sm text-green truncate">{p.name}</div>
                <div className="text-sm text-green-dim truncate">{p.company}</div>
              </div>
            </div>
          );
        })}
      ...
      <div className="flex gap-1 flex-shrink-0">
        <a
          ...
          className="btn-terminal text-xs px-2"
        >
          MATCH →
        </a>
        <button className="btn-terminal text-xs px-2 text-red border-red/40" onClick={onClear}>
          CLEAR
        </button>
      </div>
    </div>
